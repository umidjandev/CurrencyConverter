...
import com.najdimu.currencyconverter.data.models.Rates
...
class MainViewModel @ViewModelInject constructor(
    ...
    fun convert(
        ...
        viewModelScope.launch(dispatchers.io) {
            ...
            when(val rateResponse = repository.getConvert("VeY5QAdGLI1hp2UbEdX2XKwCI6EA2cvB", fromCurrency, toCurrency, fromAmount)){
                is Resource.Error -> _conversion.value = CurrencyEvent.Failure(rateResponse.message!!)
                is Resource.Success -> {
                    val rates = rateResponse.data?.result
                    println(rates)
                    if (rates == null) {
                        _conversion.value = CurrencyEvent.Failure("Unexpected error")
                    }else{
                        val convertedCurrency = round(rates?.times(100.0) ?: 0.0) / 100
